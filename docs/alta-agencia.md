...
5. El navegador dispara (best-effort) un POST a `automanize-backend` con un resumen — el equipo recibe un email
...
```
https://backend.automanize.com/webhook/onboarding-completado
```

(este webhook vivía antes en n8n — n8n se apagó del todo el 18 ago 2026 y se migró a `automanize-backend`, ver `automanize-backend/src/onboarding/notificar.js`) con el payload:
...
`automanize-backend` recibe el webhook, arma el email (mismo asunto/cuerpo que tenía el workflow de n8n) y lo manda con la Gmail API (`enviarEmailGmail`, ya usada en el resto del backend) a `martinmunozinfo@gmail.com`, con un enlace a `panel-interno.html` para ver el detalle. Si el webhook falla, el envío del formulario funciona igual (es best-effort) pero no llega ningún aviso.

**Los archivos llegan al aviso ya adjuntos desde el propio formulario, sin pasar por Supabase en este paso**: `alta-agencia.html` lee cada archivo seleccionado con `FileReader` y lo manda codificado en base64 dentro del mismo POST al webhook (`archivos: [{nombre, content}]`, y `logo: {nombre, content}` si hay logo). El backend solo tiene que montar el email con esos adjuntos — no necesita ninguna key de Supabase ni ir a buscar nada a Storage. (Los archivos se guardan también en el bucket `onboarding-archivos` por separado, en paralelo, para que el panel admin pueda seguir mostrándolos — ver más abajo — pero esa subida y el envío del email son dos caminos independientes.)
...
> El workflow de n8n que hacía esto antes (con Resend) está retirado — `docs/n8n-onboarding-completado.json` se eliminó del repo, ya no aplica.
